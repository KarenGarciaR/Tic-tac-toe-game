...
public class JuegoGato extends JFrame implements ActionListener {
    ...
    int turno=-1;// a quien le corrsponde le turno del gato
    JLabel mensaje;//para ver a quien le toca al momento de que este jugando
    Color colorB; // Cuando cambia 
    ...
    public JuegoGato(){
        ...
        mensaje.setFont(new Font("Agency FB",Font.BOLD,18)); //tamaño de letra
        this.add(mensaje);
    ...
    for(int i=0; i<3; i++){
        for(int j=0; j<3; j++){
            ...
            tablero[i][j].setBounds((i*120)+10,(j*120)+10,120,120);//posiciones
            tablero[i][j].setBackground(Color.pink);
            this.add(tablero[i][j]);
        }
    }
        colorB = tablero[0][0].getBackground(); //color de inicio
    ...
    public void actionPerformed(ActionEvent e) {
            if(e.getSource () ==iniciar){ //si, entonces
                turno = 0;
                ...
                JButton boton = (JButton)e.getSource();//detecta y guarde en un boton el elemntoo en donde ocurre un elemnto y se lamacena en un boton
    ...
    public void limpiar(){//metodo para limpoiar el tablero por nosotros cada vez que se quiera inicar el juego
        ...
        for(int i=0; i<3; i++){
            for (int j=0; j<3; j++){
                ...
                tablero[i][j].setText("");//limpiamos el texto (X o O)
